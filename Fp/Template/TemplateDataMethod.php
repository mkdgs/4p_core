...
class TemplateDataMethod {
	...
	public static function getIf($t, $args=null) {
		...
		$o = &$t;
		foreach ( $args as $v ) {
			if ( is_array($o->vars) AND array_key_exists($v, $o->vars) ) $o = &$o->vars[$v];
			else return null;
		}
		return $o;
	}
	...
	public static function toArray($t) {
		if ( !is_array($t->vars) ) return array(self::v($t));
		$a = array();
	...
	public static function r($t) {
		...
		$c = clone $t;
		foreach ( $args as $d ) {
			if ( $d instanceof TemplateData ) $c->vars = preg_replace(array("#\{$d->key\}#", '#%7B.'.$d->key.'%7D#'), $d->v(), $c->vars);
			elseif ( is_array($d) ) {
				foreach ( $d as $k => $v ) $c->vars = preg_replace(array("#\{$k\}#", '#%7B.'.$k.'%7D#'), $v, $c->vars);
			}
		}
		return $c;
	}
	...
	public static function groupBy($t, $group_key, $group_function = null) {
		if ( !$t instanceof TemplateData ) return null;
	...
	public static function iteratePosition($t) {
		if( $t->i_position === null ) return $t->i_position = $t->i_total - $t->i_iterate;
		return $t->i_position;
	}
