...
abstract class Core {
	...
	public function db() {
		...
		return Db::get_link($this->global_private['db'][0]['id']);
	}
	...
	protected function login() {
		...
		if ( !$auth->isAuth() ) {
			...
			if ( isset($_GET['logout']) || Filter::id('a',$_GET) == 'logout' ) {
				$uid = $auth->uid();
				$auth->logout();
	...
	private static function autoload_psr_search(Core $O, $dir, $className,$prefix=null) {
