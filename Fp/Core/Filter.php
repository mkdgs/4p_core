...
class Filter {
    ...
    private static function getVal($var = null, $array = null) {
        ...
        if (get_magic_quotes_gpc() && is_string($var))
    ...
    public static function iso8859_1($var, $array = null) {
        ...
        if ($enc == "ISO-8859-1")
            ...
            return mb_convert_encoding($var, "ISO-8859-1", $enc);
    }
    ...
    public static function int($var, $array = null, $default = null, $min = null, $max = null) {
        ...
        if ($var === null)
            return $default;
        if (strlen((string) $var))
            return (int) $var; // on ne perturbe pas la valeur par defaut
        return $default;
    }
    ...
    public static function float($var, $array = null, $default = null) {
        ...
        if ($var === null)
            return $default;
        if (strlen((string) $var))
            return (float) $var; // on ne perturbe pas la valeur par defaut
        return $default;
    }
    ...
    public static function b64Compress($var, $array = null, $default = '') {
        $var = self::getVal($var, $array);
        if ($var)
            return strtr(base64_encode(gzdeflate(serialize($var))), '+/=', '-_,');
        return self::setVal($var, $default);
    }
    ...
    public static function b64Decompress($var, $array = null, $default = '') {
        $var = (string) self::getVal($var, $array);
        if ($var)
            return unserialize(gzinflate(base64_decode(strtr($var, '-_,', '+/='))));
        return self::setVal($var, $default);
    }
    ...
    public static function mysqlDate($var, $array = null, $default = '') {
        $var = trim((string) self::getVal($var, $array, $default));
        $var = str_replace('/', '-', $var);
        if (!preg_match('#([0-9]{2,4})-([0-9]{1,2})-([0-9]{1,2})#', $var, $m))
            return $default;
        // validate day
        if (intval($m[1]) < 1 || intval($m[2]) > 12)
            return $default;
        //validate month
        if (intval($m[2]) < 1 || intval($m[3]) > 31)
            return $default;
    ...
    public static function html($var, $array = null, $default = '') {
        ...
        foreach ($dom->childNodes as $p) {
            if ($p->nodeType == XML_PI_NODE)
                $dom->removeChild($p); // remove hack for utf-8
    ...
    public static function htmlToText($var, $array = null, $default = '', callable $functionConvertNode = null) {
        $var = (string) self::getVal($var, $array, $default);
        //$var = Filter::utf8($var);
